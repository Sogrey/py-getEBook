...
def get_ChartTxt(url, title, num, totalNum, encoding):
    # print('本章节地址： ' + url)
...
def mergeFiles(title, encoding):
    ...
    with open(r"%s/%s.txt" % (title, title), "w", encoding=encoding) as outFile:
        outFile.write(res)
        outFile.close()
    print('整书《%s》合并完成，总大小：%d' % (title, len(res)))
    return
...
def thread_getOneBook(url, encoding):
    ...
    for t in textlist:
        ...
        # 判断是否有感言
        # if re.search(r'.*?章', chart_title) is None:
        #     print('抓到作者感言，跳过...')
        #     continue
    ...
    for i in charts_url:
        ...
        url = baseUrl+i
        p.apply_async(get_ChartTxt, args=(url, title, num, totalNum, encoding))
        num += 1
